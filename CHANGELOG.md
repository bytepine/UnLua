...
- fix(compat): Android NDK clang — `FSoftObjectPtr::Set` 不再导出 `operator=` 成员指针（重载无法 C 转，MSVC 4.26–5.5 / NDK clang 均中招），改走 Lua C 封装赋值；`FEnumRegistry` / `FClassRegistry` 析构 range-for 改引用，避免 `-Werror,-Wrange-loop-construct`
